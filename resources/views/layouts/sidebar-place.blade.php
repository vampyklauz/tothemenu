
<div class="page-sidebar col-xs-2 no-padding">
	<div class="col-xs-12 widget-container-col no-padding">
		<div class="widget-box widget-color-blue">
			<!-- #section:custom/widget-box.options -->
			<div class="widget-header">
				<h5 class="widget-title bigger lighter">
					<!-- <i class="ace-icon fa fa-table"></i>
					Tables & Colors -->
				</h5>
			</div>

			<!-- /section:custom/widget-box.options -->
			<div class="widget-body">
				<div class="widget-main no-padding">
					<ul class="county m-top-20">
						@foreach ($counties as $location)
							<li><a href="{{ url('page/')."/".$location->id }}" data-state="{{ $location->county_code }}">{{ $location->county_name }}</a></li>
						@endforeach
						<!-- <li><a href="page/Bergen" data-state="BN">Bergen</a></li>
						<li><a href="page/Burlington" data-state="BU">Burlington</a></li>
						<li><a href="page/Camden" data-state="CN">Camden</a></li>
						<li><a href="page/Cape May" data-state="CM">Cape May</a></li>
						<li><a href="page/Cumberland" data-state="CD">Cumberland</a></li>
						<li><a href="page/Essex" data-state="EX">Essex</a></li>
						<li><a href="page/Gloucester" data-state="GR">Gloucester</a></li>
						<li><a href="page/Hudson" data-state="HN">Hudson</a></li>
						<li><a href="page/Hunterdon" data-state="HU">Hunterdon</a></li>
						<li><a href="page/Mercer" data-state="MR">Mercer</a></li>
						<li><a href="page/Middlesex" data-state="MX">Middlesex</a></li>
						<li><a href="page/Monmouth" data-state="MH">Monmouth</a></li>
						<li><a href="page/Morris" data-state="MS">Morris</a></li>
						<li><a href="page/Ocean" data-state="ON">Ocean</a></li>
						<li><a href="page/Passaic" data-state="PX">Passaic</a></li>
						<li><a href="page/Salem" data-state="SM">Salem</a></li>
						<li><a href="page/Somerset" data-state="ST">Somerset</a></li>
						<li><a href="page/SX" data-state="SX">Sussex</a></li>
						<li><a href="page/Union" data-state="UN">Union</a></li>
						<li><a href="page/Warren" data-state="WN">Warren</a></li> -->
					</ul>
				</div>
			</div>
		</div>
	</div><!-- /.span -->
</div>